...
describe('outdoorTimeOfDay', () => {
  ...
  it('pushes night fog past the play space and boosts target readability', () => {
    ...
    expect(night.targetReadabilityBoost).toBeGreaterThanOrEqual(0.5)
    expect(resolved.targetReadabilityBoost).toBe(night.targetReadabilityBoost)
    expect(night.toneMappingExposure).toBeGreaterThan(1.0)
    expect(night.hemisphere.intensity).toBeGreaterThan(1.0)
    expect(night.fillLight?.intensity).toBeGreaterThan(1.5)
    expect(night.grassTint).toBe('#4a6450')
  })
})
