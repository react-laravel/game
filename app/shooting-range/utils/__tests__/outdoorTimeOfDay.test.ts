...
describe('outdoorTimeOfDay', () => {
  ...
  it('pushes night fog past the play space and boosts target readability', () => {
    ...
    expect(night.targetReadabilityBoost).toBeGreaterThanOrEqual(0.45)
    expect(resolved.targetReadabilityBoost).toBe(night.targetReadabilityBoost)
    expect(night.toneMappingExposure).toBeGreaterThan(0.95)
    expect(night.hemisphere.intensity).toBeGreaterThan(0.7)
    expect(night.fillLight?.intensity).toBeGreaterThan(0.8)
  })
})
