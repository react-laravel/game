...
APP_ROOT="${APP_ROOT:-/var/www/game}"
...
pm2 startOrReload "$CURRENT/ecosystem.config.cjs" --only game-nextjs --update-env

healthy=false
for _ in {1..15}; do
  api_status="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 2 http://127.0.0.1:3011/api/user || true)"
  if curl -fsS --max-time 2 http://127.0.0.1:3011/ >/dev/null \
    && [ "$api_status" = "401" ] \
    && timeout 2 bash -c '</dev/tcp/127.0.0.1/8082' 2>/dev/null; then
    healthy=true
    break
  fi
  sleep 1
